...
public class RequestPartArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public boolean isSupported(Parameter parameter) {
		...
		return annotation != null && (
			MultipartFile.class.equals(paramterType)
			||	Part.class.equals(paramterType)
			||	(
				paramterType.isArray() && paramterType.getComponentType().equals(MultipartFile.class)	
			)
		);
	}
	...
	public Object argumentResolve(Parameter parameter, HttpServletRequest req, HttpServletResponse resp)
		...
		if(req instanceof StandardMultipartHttpServletRequest) {
			...
			if(required && parameterValue == null) {
				//필수파트인 경우, 필수파일이 누락된 경우 -> 400
				throw new BadRequestException("필수 파일이 업로드 되지 않았습니다.");	
			}
			return parameterValue;
		}
		else {
			throw new BadRequestException("파일이 포함된 멀티파트 요청이 아닙니다.");
		}
	}
